...
The number of hidden units and dropout rate are configurable from the command line.
...
import json              # saving the label map as a JSON file
...
def build_label_map(labels: np.ndarray) -> dict[str, int]:
    ...
    return {label: idx for idx, label in enumerate(unique)}  # e.g. {"entertainment": 0, ...}
...
def train(args):
    ...
    label_map = build_label_map(train_y_str)           # create string->int mapping from training labels
    train_y = encode_labels(train_y_str, label_map)    # convert training labels to integers
    dev_y   = encode_labels(dev_y_str,   label_map)    # convert dev labels using the same mapping
    ...
    # Create the neural network and move it to the chosen device
    model = FeedForwardClassifier(
        input_dim=train_X.shape[1],      # number of input features = embedding dimension
        hidden_size=args.hidden_size,
        num_classes=len(label_map),      # one output neuron per topic class
    ...
    best_dev_acc = 0.0         # track the best accuracy seen so far on the dev set
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        avg_loss = total_loss / len(train_dataset)  # average loss over all training examples
    ...
    # Save label map so the evaluator can convert predicted integers back to topic names
    ...
    if args.plot:
        ...
        # Right y-axis: dev accuracy (shares the same x-axis)
        ax2 = ax1.twinx()
        ...
        fig.tight_layout()                             # prevent labels from being cut off
        os.makedirs(os.path.dirname(args.plot) or ".", exist_ok=True)
        fig.savefig(args.plot, dpi=150)                # save as a high-resolution PNG
...
def main():
    parser = argparse.ArgumentParser(
        description="Train a feed-forward classifier on FastText sentence embeddings."
    )
    parser.add_argument("--train_embeddings", required=True,
                        help="Path to train .npz file (from sentence_embeddings.py).")
    parser.add_argument("--dev_embeddings", required=True,
                        help="Path to dev .npz file.")
    parser.add_argument("--output_model", required=True,
                        help="Where to save the best model weights (.pt).")
    parser.add_argument("--output_labels", required=True,
                        help="Where to save the label->index JSON mapping.")
    ...
    parser.add_argument("--plot",        default=None,
                        help="Optional path to save the training curve as a .png file.")
