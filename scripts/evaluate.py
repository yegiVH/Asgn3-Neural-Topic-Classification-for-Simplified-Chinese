...
# Evaluation
# ---------------------------------------------------------------------------
def evaluate(model, device, X: np.ndarray, y_int: np.ndarray, label_names: list[str], split_name: str) -> None:
    ...
    cm = confusion_matrix(y_int, preds, len(label_names)) 
...
def main():
    ...
    parser.add_argument("--embeddings", nargs="+", required=True, help="One or more .npz files to evaluate on.")
    parser.add_argument("--hidden_size", type=int, default=128, help="Hidden size used during training (default: 128).")
